class Draw {
    ...
    OVERLAY_OFFSET_Y = 180
    RATIO = 600 / 338
    RATE = 1
    WIDTH = 600
    HEIGHT = 338
    ORIGIN_IMG_WIDTH = 600
    ...
    isDark = document.querySelector('#is-dark')
    isWatermark = document.querySelector('#is-add-watermark')
    isOverlay = document.querySelector('#is-add-overlay')
    cropper = document.querySelector('.cropper')
    handlers = document.querySelectorAll('.cropper div')
    selected = document.querySelector('.selected')
    downloadBtn = document.getElementById('download')
    fontInput = document.querySelector('#font')
    isPing = document.querySelector('#is-ping')

    mousePosition = { x: 0, y: 0, isDown: false }
    offset = [0, 0]

    originImageRatio = 1

    watermark = new Image
    watermarkPing = new Image
    overlay = new Image
    overlayDark = new Image
    overlayPing = new Image

    constructor() {
        this.input.addEventListener('keyup', () => {
            this.draw()
        })
        this.isDark.addEventListener('change', () => {
            this.draw()
        })
        this.isWatermark.addEventListener('change', () => {
            this.draw()
        })
        this.isOverlay.addEventListener('change', e => {
            this.draw()
        })
        this.originImg.addEventListener('load', () => {
            let { naturalWidth, naturalHeight } = this.originImg
            this.originImageRatio = naturalWidth / this.ORIGIN_IMG_WIDTH
            let height = this.originImageRatio / naturalHeight
            this.selectedWidth = this.ORIGIN_IMG_WIDTH
            this.cropper.style.width = this.ORIGIN_IMG_WIDTH + 'px'
            this.cropper.style.height = height + 'px'
            document.querySelector('.origin-img > div').style.width = this.ORIGIN_IMG_WIDTH + 'px'
            document.querySelector('.origin-img > div').style.height = height + 'px'
            this.selected.style.width = this.ORIGIN_IMG_WIDTH + 'px'
            this.selected.style.height = (this.ORIGIN_IMG_WIDTH / this.RATIO) + 'px'
        ...
        this.selected.addEventListener('mousedown', e => {
            this.dragStartX = e.clientX
            this.dragStartY = e.clientY
            this.mousePosition.isDown = true
            this.offset = [
                this.selected.offsetLeft - e.clientX,
                this.selected.offsetTop - e.clientY
            ]
            console.log('123', this.offset)
        }, true)
        this.selected.addEventListener('mouseup', e => {
            this.mousePosition.isDown = false
        }, true)
        this.selected.addEventListener('mousemove', e => {
            e.preventDefault()
            if (this.mousePosition.isDown) {
                ...
                let left = this.mousePosition.x + this.offset[0]
                let top = this.mousePosition.y + this.offset[1]
                // if (
        ...
        document.addEventListener('keydown', e => {
            let top, left
            let topMax = this.originImg.height
            let leftMax = this.originImg.width
            switch (e.keyCode) {
                case 38: // up
                    e.preventDefault()
                    this.selectedWidth += 5
                    this.selected.style.width = this.selectedWidth + 'px'
                    this.selected.style.height = this.selectedWidth / this.RATIO + 'px'
                    break
                case 40: // down
                    e.preventDefault()
                    this.selectedWidth -= 5
        ...
        document.addEventListener('paste', (e) => {
            ...
            for (let index in items) {
                ...
                if (item.kind === 'file') {
                    ...
                    reader.onload = (event) => {
                        this.originImg.src = event.target.result
                    }
        ...
        document.querySelector('#download-2').addEventListener('click', e => {
            let a = document.createElement('a')
        ...
        this.fontInput.addEventListener('change', e => {
            if (!isNaN(this.fontInput.value)) {
                this.fontSize = this.fontInput.value
                this.draw()
            }
        })
        this.isPing.addEventListener('change', e => {
            this.draw()
        })
    ...
    drawImage() {
        ...
        let width = this.selectedWidth * this.originImageRatio
        let height = this.selectedWidth / this.RATIO * this.originImageRatio
        let left = (this.offset[0] + this.mousePosition.x) * this.originImageRatio
        let top = (this.offset[1] + this.mousePosition.y) * this.originImageRatio
        console.log(top, left, width, height)
        console.log('ratio', this.originImageRatio)
        console.log('offse', this.offset)
        this.ctx.drawImage(this.originImg, left, top, width, height, 0, 0, 600, 338)
    }
    ...
    handleFile() {
        ...
        reader.addEventListener('load', () => {
            this.originImg.src = reader.result
        }, false)
        if (file) {
